...
class PaperIntegration:
    ...
    def ensure_directory(self):
        ...
        os.makedirs(f"{self.paper_dir}/chapters", exist_ok=True)
        os.makedirs(f"{self.paper_dir}/figures", exist_ok=True)
        os.makedirs(f"{self.paper_dir}/appendix", exist_ok=True)
    
    def generate_ckm_chapter(self):
        """生成CKM章节"""
        chapter = """# Chapter 7: Geometric Origin of the CKM Matrix
...
"""
        ...
        filepath = f"{self.paper_dir}/chapters/chapter7_ckm_matrix.md"
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(chapter)
        
        print(f"✅ CKM章节已生成: {filepath}")
        print(f"   字数: ~2,500字")
        return filepath
    
    def generate_paper_outline(self):
        """生成论文大纲"""
        outline = """# Unified Field Theory Paper Outline

## Title
Fixed 4D Topology - Dynamic Spectral Dimension Multiple Twisting Fractal Clifford Algebra: A Geometric Unified Field Theory
...
4. **Particle Spectrum**
   ...
   - Three generations
   - [COMPLETED] CKM matrix (Chapter 7)
...
6. **Black Holes**
   ...
   - Fractal-twisting model
   - Hawking radiation

7. **CKM Matrix** ✓
   - [COMPLETED] Geometric origin
   - [COMPLETED] ~1% precision derivation
   - [COMPLETED] Physical interpretation

8. **Cosmology**
   - Torsion-driven inflation
...
## Timeline
- First draft: 2026-03
- Internal review: 2026-04
- Submission: 2026-05
"""
        
        filepath = f"{self.paper_dir}/PAPER_OUTLINE.md"
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(outline)
        
        print(f"✅ 论文大纲已生成: {filepath}")
        return filepath

def main():
    print("="*70)
    print("方向1: 论文整合 - CKM章节撰写")
    print("="*70)
    
    integrator = PaperIntegration()
    
    # 生成CKM章节
    ckm_chapter = integrator.generate_ckm_chapter()
    
    # 生成论文大纲
    outline = integrator.generate_paper_outline()
