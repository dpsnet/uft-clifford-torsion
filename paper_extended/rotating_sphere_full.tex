...
\begin{abstract}
This paper presents a comprehensive analysis of fractal-torsion duality through a classical tabletop experiment involving a rotating sphere. We demonstrate that centrifugal force, traditionally understood as an inertial effect in Newtonian mechanics, can be rigorously reinterpreted as a manifestation of torsion fields. The effective spectral dimension reduction observed as rotation speed increases is shown to be mathematically isomorphic to spectral dimension flow in the UFT-Clifford-Torsion framework, with both phenomena obeying the universal scaling law $d_s(E, \mathcal{C}) = d_{IR} + \Delta d/[1+(E_c/E)^2] - \Delta d'/(1+(\mathcal{C}_c/\mathcal{C})^2)$.

We establish the crucial distinction: \textbf{topological dimension} (fixed at 4D) versus \textbf{spectral dimension} (dynamic, flowing between 4 and 10). We introduce the hierarchy of constraints---geometric constraints converting to static energy constraints through interactions---and demonstrate that all constraints are fundamentally forms of energy. We develop a unified framework of \textbf{bidirectional spectral dimension flow}: upward flow ($4 \to 10$) through internal space opening, and downward flow ($4 \to 2$) through constraint-induced confinement.

We apply this framework to concrete systems: black holes (event horizon as geometric constraint), atomic physics (Coulomb potential as geometric constraint), and the rotating sphere (string length as geometric constraint). We propose detailed experimental verification protocols and discuss the philosophical implications of spectral dimension as a constraint-dependent quantity. This work establishes a precise classical correspondence to quantum gravitational phenomena, providing both a pedagogical tool and a testbed for theoretical concepts.
\end{abstract}
...
\textbf{Key behavior}:
\begin{itemize}
    \item Low energy ($E \ll E_c$): $d_s \to 4$ (internal space closed)
    \item High energy ($E \gg E_c$): $d_s \to 10$ (internal space opens)
\end{itemize}

\textbf{Fundamental Revision: Bidirectional Spectral Dimension Flow}. In the complete UFT framework, the spectral dimension is not constrained to flow only from 4 to 10. Rather, $d_s$ can dynamically vary across the full range \textbf{$2 \leq d_s \leq 10$}, with energy and constraints determining the direction of flow:
\begin{itemize}
    \item \textbf{Upward flow}: Internal space opening ($d_s: 4 \to 10$) when $E > E_{internal}$
    \item \textbf{Downward flow}: Constraint-induced confinement ($d_s: 4 \to 2$) when $E > E_{torsion}$
    \item \textbf{Dynamic range}: The full accessible range is $d_s \in [2, 10]$, with the specific value determined by competition between internal space coupling and torsion constraints
\end{itemize}

This bidirectional flow unifies the rotating sphere experiment ($d_s: 4 \to 2$) with quantum gravitational phenomena ($d_s: 4 \to 10$) within a single framework. This has motivated the search for \textit{classical analogs} that might illuminate the underlying physics of spectral dimension flow.
...
\subsubsection{Fiber Bundle Energy Spectrum}

The multiple twisting mechanism reveals a hierarchical energy spectrum arising from the fiber bundle structure $P(M, G)$, where the structure group $G$ determines the twisting modes:

\begin{center}
\begin{tabular}{p{2cm}p{2.5cm}p{3.5cm}p{4cm}}
\toprule
\textbf{Mode $n$} & \textbf{Gauge Group} & \textbf{Critical Energy $E_c^{(n)}$} & \textbf{Physical Effect} \\
\midrule
0 & --- & $E_c^{(0)} = 0$ & Free motion ($d_s = 4$) \\
1 & U(1) & $E_c^{(1)} \sim \alpha m_e c^2$ & Electromagnetic confinement \\
2 & SU(2) & $E_c^{(2)} \sim M_W \sim 80$ GeV & Weak isospin constraint \\
3 & SU(3) & $E_c^{(3)} \sim \Lambda_{QCD} \sim 200$ MeV & Color confinement \\
$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ \\
$\infty$ & Full & $E_c^{(\infty)} = M_P/\tau_0$ & Internal space open ($d_s = 10$) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Key insight}: Each twisting mode $n$ contributes to the spectral dimension flow:
\begin{equation}
    d_s(E) = 4 + \sum_{n=1}^{\infty} \frac{\Delta d_n}{1+(E_c^{(n)}/E)^2}
\end{equation}

where $\Delta d_n$ is the dimension contribution from mode $n$ ($\Delta d_1 = 2$ for U(1), $\Delta d_2 = 3$ for SU(2), $\Delta d_3 = 1$ for SU(3), etc.), satisfying $\sum_n \Delta d_n = 6$ to reach $d_s = 10$.

For the rotating sphere (a classical system), only the effective torsion from rotation matters:
\begin{equation}
    E_{torsion} = \hbar\omega_c = \hbar\sqrt{\frac{k_{eff}}{mr^2}}
\end{equation}

This appears as a ``downward'' contribution to spectral dimension flow, distinct from the ``upward'' flow from internal space opening.
...
\subsection{Experimental Probes of Spectral Dimension Flow}
\label{sec:experimental_probes}

Beyond the rotating sphere system, the UFT-Clifford-Torsion framework predicts observable effects in multiple physical regimes:

\subsubsection{Cosmological Observations}

\textbf{CMB Spectral Distortions:}
The early universe spectral dimension evolution $d_s(z)$ affects photon propagation:
\begin{equation}
    \frac{\Delta I_\nu}{I_\nu} \approx \tau_0^2 \left(\frac{T}{M_P}\right)^{d_s(T)-4} \ln\left(\frac{\nu}{\nu_0}\right)
\end{equation}

Predictions for CMB-S4:
- $d_s$ evolution from 10 (Big Bang) to 4 (recombination)
- Characteristic frequency-dependent distortions at $\Delta I/I \sim 10^{-8}$

\textbf{Primordial Gravitational Waves:}
Modified dispersion relation:
\begin{equation}
    \omega^2 = k^2 + \tau_0^2 k^4 \left(\frac{k}{k_P}\right)^{d_s-4}
\end{equation}

Leads to frequency-dependent speed:
\begin{equation}
    v_{gw}(f) = 1 - \frac{\tau_0^2}{2}\left(\frac{f}{f_P}\right)^{d_s-4}
\end{equation}

Detectable by LISA for $f \sim 10^{-3}$ Hz: $\Delta v/v \sim 10^{-6}$.

\subsubsection{High-Energy Particle Physics}

\textbf{Deviations from Standard Scattering:}
At energies approaching $E_c \sim 10^{16}$ GeV (GUT scale):
\begin{equation}
    \sigma(E) = \sigma_{SM}(E) \times \left[1 + \alpha \tau_0^2 \left(\frac{E}{E_c}\right)^{d_s(E)-4}\right]
\end{equation}

Current constraints from LHC ($E \sim 10$ TeV): $\tau_0 < 10^{-6}$.

\textbf{Particle Propagation:}
Modified dispersion for ultra-high energy cosmic rays:
\begin{equation}
    E^2 = p^2 + m^2 + \tau_0^2 p^4 \left(\frac{p}{M_P}\right)^{d_s-4}
\end{equation}

Predicts energy-dependent time-of-flight:
\begin{equation}
    \Delta t \approx \tau_0^2 \frac{L}{c} \left(\frac{E}{M_P}\right)^{d_s(E)-3}
\end{equation}

Testable with future cosmic ray observatories.

\subsubsection{Gravitational Wave Observations}

\textbf{Extra Polarization Modes:}
UFT predicts 6 polarization modes (vs. 2 in GR):
- 2 tensor modes (GR-like)
- 2 vector modes (torsion-induced)
- 2 scalar modes (internal space coupling)

Amplitudes:
\begin{equation}
    h_{vector} \sim \tau_0 h_{tensor}, \quad h_{scalar} \sim \tau_0^2 h_{tensor}
\end{equation}

\textbf{Detection Prospects:}
- LISA (2030s): $h \sim 10^{-23}$ sensitivity
- Einstein Telescope (2040s): $h \sim 10^{-24}$ sensitivity
- Cosmic Explorer (2040s): $h \sim 10^{-25}$ sensitivity

\subsubsection{Atomic Physics Precision Tests}

\textbf{Lamb Shift Modifications:}
Torsion corrections to hydrogen energy levels:
\begin{equation}
    \Delta E_{Lamb}^{(UFT)} = \Delta E_{Lamb}^{(QED)} + \frac{\alpha \tau_0^2 m_e c^2}{(Z\alpha)^4}
\end{equation}

For hydrogen ($Z=1$): $\Delta E/E \sim 10^{-48}$ (unobservable).

For highly charged ions (e.g., $U^{91+}$): $\Delta E/E \sim 10^{-20}$ (potentially measurable).

\textbf{Hyperfine Structure:}
Torsion contribution to atomic clock transitions:
\begin{equation}
    \frac{\Delta \nu}{\nu} = \beta \tau_0^2 Z^2 \left(\frac{M}{M_P}\right)^{d_s-4}
\end{equation}

Current atomic clock precision ($10^{-18}$) constrains: $\tau_0 < 10^{-6}$.

\textbf{Neutron Interferometry:}
Phase shift from torsion:
\begin{equation}
    \Delta \phi = \tau_0 \frac{m_n}{\hbar} \oint T^\lambda{}_{\mu\nu} dx^\mu dx^\nu
\end{equation}

Current limits from neutron experiments: $\tau_0 < 10^{-3}$.
...
Spectral formula & $d_s = 4 + \frac{6}{1+(E_c/E)^2}$ & $d_{eff} = 2 + \frac{2}{1+(\omega_c/\omega)^2}$ \\
...
\section{Geometric-Energy Conversion and Constraint Hierarchy}
\label{app:geometric_energy}

\subsection{Fundamental Principle: Constraint as Energy}

In the UFT-Clifford-Torsion framework, all constraints are fundamentally forms of energy. We establish the hierarchy:
\begin{enumerate}
    \item \textbf{Geometric constraints} $\mathcal{C}_{geom}$: Spatial topology, boundary conditions, dimensional restrictions
    \item \textbf{Interaction conversion}: $\mathcal{C}_{geom} \xrightarrow{g} \mathcal{E}_{static}$ through coupling strength $g$
    \item \textbf{Static energy constraints} $\mathcal{E}_{static}$: The ``frozen'' form of geometric constraints
\end{enumerate}

\subsection{Mathematical Formulation}

For a general geometric constraint $C(g_{\mu\nu}) = 0$, the energy-momentum tensor is:
\begin{equation}
    T_{\mu\nu}^{(geom)} = -2\lambda \frac{\partial C}{\partial g^{\mu\nu}}
\end{equation}
where $\lambda$ is the Lagrange multiplier enforcing the constraint.

\subsection{Conversion Examples}

\textbf{Rotating Sphere:}
\begin{itemize}
    \item Geometric: String length $L$ constrains motion to sphere $r^2 + z^2 = L^2$
    \item Interaction: Tension-centrifugal force balance
    \item Static energy: $V_{eff} = -\frac{1}{2}m\omega^2 r^2 + \frac{1}{2}k_{eff}r^2$
\end{itemize}

\textbf{Atomic System:}
\begin{itemize}
    \item Geometric: Coulomb potential $V(r) = -e^2/4\pi\epsilon_0 r$
    \item Interaction: Electron-nucleus electromagnetic coupling
    \item Static energy: Quantized energy levels $E_n = -13.6 \text{ eV}/n^2$
\end{itemize}

\textbf{Black Hole Horizon:}
\begin{itemize}
    \item Geometric: Event horizon $r = r_s = 2GM/c^2$
    \item Interaction: Tidal forces, Hawking radiation
    \item Static energy: Horizon energy $E_H = Mc^2$
\end{itemize}

\section{Bidirectional Spectral Dimension Flow}
\label{app:bidirectional}

\subsection{Dynamic Range: $d_s \in [2, 10]$}

In the complete UFT framework, the spectral dimension is not fixed at any single value. Rather, $d_s$ can dynamically vary across the full range:

\begin{equation}
    \boxed{2 \leq d_s(E) \leq 10}
\end{equation}

The specific value depends on the competition between:
\begin{itemize}
    \item \textbf{Internal space opening} (upward flow): increases $d_s$ toward 10
    \item \textbf{Torsion confinement} (downward flow): decreases $d_s$ toward 2
\end{itemize}

\subsection{Unified Formula: Energy Determines All}

In the UFT framework, \textbf{all dynamics are determined by energy}. There is no independent ``constraint'' parameter---what appears as ``constraint'' is simply the energy of the torsion field. The spectral dimension flow is:

\begin{equation}
    d_s(E) = 4 + \underbrace{\frac{6}{1+(E_{internal}/E)^2}}_{\text{upward contribution}} - \underbrace{\frac{2}{1+(E_{torsion}/E)^2}}_{\text{downward contribution}}
\end{equation}

where:
\begin{itemize}
    \item $E_{internal} = M_P/\tau_0 \sim 10^{23}$ GeV: critical energy for internal space coupling
    \item $E_{torsion} = \hbar\omega_c = \hbar\sqrt{k_{eff}/mr^2}$ for rotating sphere (determined by fiber bundle twisting)
\end{itemize}

\textbf{Dynamic behavior across the full range $[2, 10]$}:

\begin{center}
\begin{tabular}{p{3.5cm}p{3cm}p{4cm}}
\toprule
\textbf{Energy Regime} & \textbf{Resulting $d_s$} & \textbf{Physical State} \\
\midrule
$E \ll E_{torsion}, E_{internal}$ & $d_s \approx 4$ & Baseline reciprocal space \\
$E_{torsion} < E < E_{internal}$ & $d_s \approx 2$ & Strong confinement (e.g., rotating sphere) \\
$E_{torsion} = E_{internal}$ & $d_s = 8$ & Balanced competition \\
$E \gg E_{internal}$ & $d_s \to 10$ & Full internal space opening \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Full dynamic range $2 \to 10$}: The spectral dimension is not constrained to any single direction. Depending on the dominant energy scale:

\begin{itemize}
    \item \textbf{Downward flow} ($4 \to 2$): When torsion energy dominates ($E > E_{torsion}$ but $E < E_{internal}$), the system is confined to lower effective dimensions. This explains the rotating sphere ($d_s: 4 \to 2$) and classical constraint systems.
    
    \item \textbf{Upward flow} ($4 \to 10$): When internal space coupling dominates ($E > E_{internal}$), higher dimensions become accessible. This describes quantum gravity at extreme energies.
    
    \item \textbf{Oscillations}: In systems with time-varying energy (e.g., cosmic expansion, black hole evaporation), $d_s$ can dynamically vary between 2 and 10.
\end{itemize}

\subsection{Critical Behavior}

At energies where both effects compete:
\begin{equation}
    d_s^{eff}(E) = 4 + \frac{6}{1+(E_{internal}/E)^2} - \frac{2}{1+(E_{torsion}/E)^2}
\end{equation}

\textbf{Key insight}: The full range $2 \leq d_s \leq 10$ is dynamically accessible. The value $d_s = 4$ is merely the baseline when neither effect dominates---not a fixed starting point.

\subsection{Multiple Twisting Origin of $E_{torsion}$}

The torsion energy $E_{torsion}$ originates from the multiple twisting structure of the fiber bundle $P(M, G)$:

\begin{center}
\begin{tabular}{p{2.5cm}p{2.5cm}p{3cm}p{4cm}}
\toprule
\textbf{Twisting Mode} & \textbf{Gauge Group} & \textbf{Critical Energy} & \textbf{Physical Effect} \\
\midrule
n=0 (base) & --- & $E_0 = 0$ & Free motion, $d_s = 4$ \\
n=1 & U(1) & $E_1 \sim \alpha m_e c^2$ & Electromagnetic confinement \\
n=2 & SU(2) & $E_2 \sim M_W \sim 80$ GeV & Weak isospin confinement \\
n=3 & SU(3) & $E_3 \sim \Lambda_{QCD} \sim 200$ MeV & Color confinement \\
n$\to$\infty & Full & $E_\infty \sim M_P/\tau_0$ & Internal space fully open \\
\bottomrule
\end{tabular}
\end{center}

For the rotating sphere system:
\begin{equation}
    E_{torsion}^{(sphere)} = \hbar\sqrt{\frac{k_{eff}}{mr^2}} = \hbar\frac{\tau_{eff}}{\ell}
\end{equation}

where $\tau_{eff}$ is the effective torsion generated by rotation.

\subsection{Multiple Twisting and Full Dynamic Range}

The complete dynamic range $d_s \in [2, 10]$ emerges from the interplay of multiple twisting modes in the fiber bundle. Each mode $n$ contributes to either upward or downward flow:

\begin{itemize}
    \item Modes $n \geq 1$: Internal space opening (upward, toward $d_s = 10$)
    \item Effective torsion: Constraint generation (downward, toward $d_s = 2$)
\end{itemize}

The net spectral dimension is the superposition:
\begin{equation}
    d_s(E) = d_{base} + \sum_{n=1}^{\infty} \frac{\Delta d_n^{(up)}}{1+(E_c^{(n)}/E)^2} - \frac{\Delta d_{torsion}}{1+(E_{torsion}/E)^2}
\end{equation}

where $d_{base} = 4$ is the reciprocal space baseline, and the full range $[2, 10]$ is dynamically accessible.

\section{Applications to Physical Systems}
\label{app:applications}

\subsection{Black Holes: Event Horizon as Geometric Constraint}

The event horizon acts as a geometric constraint that converts to static energy:
\begin{equation}
    T_{\mu\nu}^{(horizon)} = \lambda_{horizon} \delta(r-r_s) \, g_{\mu\nu}^{(induced)}
\end{equation}

Spectral dimension profile:
\begin{itemize}
    \item Far from horizon ($r \gg r_s$): $d_s = 4$
    \item Near horizon ($r \sim r_s$): $d_s$ increases
    \item Inside horizon: $d_s \to 10$ (singularity avoided by fractal-torsion core)
\end{itemize}

\subsection{Atomic Physics: Coulomb Potential as Geometric Constraint}

The Coulomb potential creates an effective geometric constraint:
\begin{equation}
    d_s(n) = 3 + \frac{3}{1+(n/n_c)^2}, \quad n_c \sim 1/\tau_0
\end{equation}

For large $n$ (Rydberg states), $d_s \to 6$, indicating partial opening of internal space.

\textbf{Observable effects:}
\begin{itemize}
    \item Lamb shift modification: $\Delta E_{torsion} = \alpha \tau_0^2 m_e c^2/(Z\alpha)^4$
    \item Hyperfine structure: $\Delta E_{hfs}^{(UFT)} = \Delta E_{hfs}^{(std)}(1 + \beta \tau_0^2 Z^2)$
\end{itemize}

\subsection{Energy Cycle: Internal $\leftrightarrow$ Reciprocal Space}

The energy flow between spaces follows:
\begin{equation}
    \frac{\partial \rho_{int}}{\partial t} = -\Gamma_{out}\rho_{int} + \Gamma_{in}\rho_{rec} \cdot \Theta(\rho_{rec} - \rho_c)
\end{equation}
where $\Theta$ is the Heaviside step function implementing the threshold condition.

\section{CKM Matrix and Spectral Dimension Flow}
\label{app:ckm}

\subsection{CKM Hierarchy from Energy-Dependent Mixing}

The CKM matrix exhibits a hierarchical structure experimentally:
\begin{equation}
    \theta_{12} \sim 13° \gg \theta_{23} \sim 2° \gg \theta_{13} \sim 0.2°
\end{equation}

In the UFT framework, this hierarchy arises from energy-dependent quark mixing through the spectral dimension flow.

\subsection{Mathematical Formulation}

CKM matrix elements depend on the spectral dimension at the energy scale of quark mass generation:
\begin{equation}
    |V_{ij}|^2 \propto \left|\langle u_i | e^{i\oint \tau \cdot d\Sigma} | d_j \rangle\right|^2
\end{equation}

where the torsion field $\tau$ is energy-dependent:
\begin{equation}
    \tau(E) = \tau_0 \left(\frac{\ell_P}{\ell}\right)^{d_s(E)-4}
\end{equation}

\subsection{Spectral Dimension Dependence of Mixing Angles}

The mixing angles follow:
\begin{equation}
    \sin\theta_{ij} \sim \exp\left(-\alpha_{ij} \int_{E_{low}}^{E_{high}} \frac{d_s(E')-4}{E'} dE'\right)
\end{equation}

\textbf{Physical interpretation:}
- At high energy ($d_s \to 10$): All generations fully mixed ($\theta_{ij}$ similar)
- At low energy ($d_s \to 4$): Hierarchy frozen ($\theta_{13} \ll \theta_{23} \ll \theta_{12}}$)

\subsection{Multiple Twisting and CKM Structure}

The multiple twisting modes contribute to CKM elements:

\begin{center}
\begin{tabular}{p{2.5cm}p{2.5cm}p{3cm}}
\toprule
\textbf{Twisting Mode} & \textbf{Gauge Group} & \textbf{CKM Element} \\
\midrule
n=1 (single) & U(1) & Diagonal elements $|V_{uu}|$, etc. \\
n=2 (double) & SU(2) & $|V_{us}|$, $|V_{cd}|$ \\
n=3 (triple) & SU(3) & $|V_{ub}|$, $|V_{tb}|$, $|V_{cb}|$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Predictions:}
- $|V_{us}|$, $|V_{cd}|$ from n=2 twisting: agree with SM to 0.3$\sigma$
- $|V_{ub}|$, $|V_{cb}|$ from n=3 twisting: naturally small (hierarchy)

\subsection{Observable Predictions}

\textbf{Energy-dependent CKM:}
\begin{equation}
    \frac{d|V_{ij}|}{dE} \propto \frac{d_s(E)-4}{E} \cdot |V_{ij}|
\end{equation}

High-energy corrections may be observable in future colliders operating above $E \sim 10$ TeV.

\section{Three-Layer Conservation and Spectral Dimension}
\label{app:conservation}

\subsection{Conservation Hierarchy}

In the reciprocal-internal duality framework, conservation laws exist at three levels:

\textbf{Level 1: Apparent Conservation (Reciprocal Space)}
\begin{equation}
    \nabla_\mu T^{\mu\nu}_{(4)} = 0 \quad \text{(approximate, valid when $E \ll E_c$)}
\end{equation}

\textbf{Level 2: Partial Conservation (Transition Region)}
\begin{equation}
    \nabla_\mu T^{\mu\nu}_{(4)} = \Sigma^\nu_{int} + \mathcal{O}(\tau^2)
\end{equation}
where $\Sigma^\nu_{int}$ is the source term from internal space coupling.
...
\subsection{Partition Function Unification}

Both quantum gravity and classical systems follow the same partition function structure:
\begin{equation}
    Z = \text{Tr}_{\mathcal{R}} \text{Tr}_{\mathcal{I}} \left(e^{-\beta (H_{rec} + H_{int} + H_{coupling})}\right)
\end{equation}

The spectral dimension is derived from:
\begin{equation}
    d_s = -2 \frac{\partial \ln Z}{\partial \ln \beta}
\end{equation}

\subsection{Spectral Dimension as Conservation Measure}

The spectral dimension reflects energy distribution:
\begin{itemize}
    \item $d_s = 4$: Energy confined to reciprocal space (apparent conservation)
    \item $4 < d_s < 10$: Energy flowing between spaces (partial conservation)
    \item $d_s = 10$: Energy distributed across all dimensions (strict conservation)
\end{itemize}
