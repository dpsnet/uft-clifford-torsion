...
The complete spectral dimension including both upward and downward flow:
\begin{equation}
    d_s(E, \mathcal{C}) = 4 + \underbrace{\frac{6}{1+(E_c/E)^2}}_{\text{upward: } 4 \to 10} - \underbrace{\frac{2}{1+(\mathcal{C}_c/\mathcal{C})^2}}_{\text{downward: } 4 \to 2}
\end{equation}

\subsection{Physical Interpretation}

\textbf{Upward flow} ($4 \to 10$): Energy opens internal space, increasing accessible dimensions.

\textbf{Downward flow} ($4 \to 2$): Constraints confine system to subspace, decreasing effective spectral dimension.

\subsection{Critical Behavior}

At intermediate energies where both effects compete:
\begin{equation}
    d_s^{eff} = 4 + 6\alpha - 2\beta
\end{equation}
where $\alpha, \beta \in [0,1]$ depend on energy and constraint strength.
