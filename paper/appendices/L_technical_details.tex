...
\subsubsection{UFT Torsion Ansatz}

In UFT, we make the specific ansatz:
\begin{equation}
...
In UFT, the spectral dimension runs as:
...
For the UFT ansatz with no matter spin:
\begin{equation}
    \Box \tau + m_\tau^2 \tau = 0
\end{equation}
...
where $b_i$ are the standard one-loop coefficients and $\delta_i(E)$ are UFT corrections:
...
\begin{table}[htbp]
\centering
\caption{Fundamental Parameters in UFT}
\begin{tabular}{@{}llll@{}}
\toprule
...
For solving UFT field equations numerically:
