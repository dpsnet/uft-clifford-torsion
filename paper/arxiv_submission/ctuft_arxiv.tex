...
This work proposes a different perspective: \textbf{fixed 4D topological dimension with dynamic spectral dimension}. The key insight is that the \textit{effective} dimensionality of spacetime depends on the energy scale and the information content of the region being probed.

The theory is built on three pillars:
\begin{enumerate}
    \item \textbf{Reciprocal-Internal Space Duality}: Observable physics lives in 4D reciprocal space, while internal structure resides in 10D spectral space;
    \item \textbf{Dynamic Spectral Dimension}: The effective dimension $d_s$ flows from 10 at Planck scale to 4 at macroscopic scales;
...
The fundamental duality is expressed through the spectral dimension flow:
\begin{align}
    d_s^{(\text{out})} &= 4(1 - f_{\text{in}}), \\
    d_s^{(\text{in})} &= 4 + 6f_{\text{in}},
\end{align}
where $f_{\text{in}}$ is the internal space energy fraction.
