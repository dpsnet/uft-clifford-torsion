...
我们的统一场论（CTUFT）与爱因斯坦的广义相对论（GR）和爱因斯坦-嘉当理论之间存在层次关系：
...
\begin{theorem}[CTUFT的广义相对论极限]
在极限 $\tau_0 \to 0$ 下，CTUFT作用量退化为爱因斯坦-希尔伯特作用量：
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{小 $\tau_0$}（等价地，大挠率）：挠率效应变得显著。CTUFT偏离广义相对论变得可观测。
\end{itemize}
...
在所有常规天体物理场景中，修正可以忽略（$\delta\tau \ll 1$），使得CTUFT与广义相对论无法区分。
...
广义相对论关注\textbf{拓扑维数}（固定为4）。CTUFT为\textbf{谱维数}添加动力学而不影响拓扑结构。
\end{proposition}
...
虽然CTUFT在经典极限下重现广义相对论，但它对引力波物理做出了不同的预言：
...
爱因斯坦寻求将所有物理几何化。广义相对论将引力几何化。CTUFT将这种几何化扩展到所有基本力。这不是对爱因斯坦工作的替代，而是其完成。
\end{remark}
...
\begin{enumerate}
    ...
    \item 在极限 $\tau_0 \to 0$ 下，CTUFT精确退化为广义相对论
...
爱因斯坦的广义相对论是CTUFT的低能、弱场极限，正如牛顿力学是广义相对论的低速极限。这是两个框架的完美整合。
