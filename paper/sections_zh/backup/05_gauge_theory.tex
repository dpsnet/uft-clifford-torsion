...
在标准模型中，规范群 $U(1) \times SU(2) \times SU(3)$ 是基于实验观测假设的。我们的理论通过扭曲覆叠映射的核分解为这些对称性提供了几何起源。
...
电磁规范群 $U(1)_{em}$ 源于单个 $Z_2$ 因子。规范势为：
\begin{equation}
...
弱相互作用规范群 $SU(2)_L$ 源于两个 $Z_2$ 因子。规范势为：
\begin{equation}
...
强相互作用规范群 $SU(3)_C$ 源于三个 $Z_2$ 因子。规范势为：
\begin{equation}
...
夸克混合（CKM）和轻子混合（PMNS）矩阵源于内空间中规范群生成元的相对取向。
...
规范耦合随能量尺度跑动，在大统一尺度 $M_{GUT} \sim 10^{16}$ GeV 处统一：
\begin{equation}
    g_1(M_{GUT}) = g_2(M_{GUT}) = g_3(M_{GUT}) = \sqrt{4\pi\tau_0}
\end{equation}

这是因为所有规范对称性在高能时都源于相同的几何结构。

\subsection{总结}

我们框架中的规范理论：
\begin{itemize}
    \item $U(1) \times SU(2) \times SU(3)$ 源于 $Z_2^5$ 核分解
    \item 耦合常数由 $\tau_0$ 确定：$g_i \propto \tau_0^{n_i/2}$
    \item CKM/PMNS混合角由质量比预言
    \item 费米子质量层次由 $\tau_0$ 的代依赖幂次解释
    \item 规范耦合在大统一尺度统一
\end{itemize}
