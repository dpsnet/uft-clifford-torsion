...
\subsection{爱因斯坦-嘉当场方程}

我们框架中的引力场方程是爱因斯坦-嘉当方程：
...
在低能极限 ($\tau_0 \to 0$) 下，这些方程退化为具有零挠率的标准爱因斯坦方程。

\subsection{六种引力波偏振模式}

与广义相对论的两种偏振模式（加号和叉号）不同，我们的理论预言了六种不同的偏振模式：

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig2_polarizations.png}
\caption{统一场论预言的六种引力波偏振模式。矢量模式 ($h_x$, $h_y$) 具有振幅 $\sim \tau_0/2$，标量模式 ($h_b$, $h_l$) 具有振幅 $\sim \tau_0^2/3$，相对于标准张量模式。}
\label{fig:polarizations}
\end{figure}

\begin{enumerate}
    \item \textbf{$h_+$ (加号)}：$e_{xx} = -e_{yy} = 1$ — 标准广义相对论模式
    \item \textbf{$h_\times$ (叉号)}：$e_{xy} = e_{yx} = 1$ — 标准广义相对论模式
    \item \textbf{$h_x$ (矢量-x)}：$e_{xz} = e_{zx} = 1$ — CTUFT预言
    \item \textbf{$h_y$ (矢量-y)}：$e_{yz} = e_{zy} = 1$ — CTUFT预言
    \item \textbf{$h_b$ (呼吸模式)}：$e_{xx} = e_{yy} = 1$ — CTUFT预言
    \item \textbf{$h_l$ (纵向模式)}：$e_{zz} = 1$ — CTUFT预言
\end{enumerate}
...
\textbf{1. LISA（空间基）}：
- 频率范围：$10^{-4}$ Hz 到 1 Hz
- 对中等质量黑洞并合敏感 ($10^4$-$10^7 M_\odot$)
- 对于 $\tau_0 = 10^{-6}$，可以以信噪比 $\sim$ 7 分辨矢量偏振
...
\subsection{与广义相对论的一致性}

当以下条件满足时，我们的理论平滑地退化为广义相对论：
\begin{enumerate}
    \item $\tau_0 \to 0$：挠率效应消失
    \item $E \ll E_c$：内部空间退耦
    \item 弱场：非线性挠率项可忽略
\end{enumerate}

所有广义相对论的经典检验（近日点进动、光线偏折、引力红移、夏皮罗延迟）在适当极限下都得到满足。

\subsection{小结}

我们框架中的引力理论：
\begin{itemize}
    \item 将广义相对论扩展为带挠率的爱因斯坦-嘉当理论
    \item 预言六种引力波偏振模式
    \item 矢量偏振具有振幅 $\sim \tau_0/2$
    \item 标量偏振具有振幅 $\sim \tau_0^2/3$
    \item 不同的传播速度导致可探测的时间延迟
    \item 在低能、弱场极限下退化为广义相对论
\end{itemize}
