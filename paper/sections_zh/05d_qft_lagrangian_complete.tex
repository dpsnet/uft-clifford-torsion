...
本节介绍从10维几何严格推导完整标准模型（加引力）拉格朗日量。从克利福德代数 $Cl(1,9)$ 出发，我们通过维度约化推导拉格朗日量中的每一项。
...
\subsection{维度约化：通过扭转膜的10D $\rightarrow$ 4D}

\subsubsection{膜-扭转机制}

在CTUFT中，10维内空间和4维互反空间是完全隔离的，仅通过产生扭转的\textbf{膜}连接：
\begin{itemize}
    \item $x^\mu$：4维互反空间（$\mu = 0,1,2,3$）——我们的物理时空
    \item 10维内空间存在于膜的``另一边''
\end{itemize}

膜作为一层膜，扭转将10维谱维流动转化为在4维拓扑空间内流动的能量。从4维视角，我们无法感知10维侧的拓扑结构；只能将其观测为10维谱流形。

4维中的有效度规接收来自膜-扭转耦合的贡献：
\begin{equation}
    ds_{4}^2 = g_{\mu\nu}dx^\mu dx^\nu + \text{（扭转修正）}
\end{equation}
...
\begin{itemize}
    \item $g_{\mu\nu}$ 是4维度规
    \item 扭转介导几何信息跨膜传输
    \item 谱维流动 $d_s(\ell)$ 将10维信息以能量形式带入4维
\end{itemize}

\subsubsection{规范对称性的涌现}

10维内空间的对称性通过膜-扭转映射在4维中显现为规范对称性：
\begin{equation}
...
\begin{enumerate}
    ...
    \item 规范部分：来自紧致化空间等距的 $U(1) \times SU(2) \times SU(3)$
    \item 费米子部分：来自马约拉纳-外尔条件的手征费米子
    \item 希格斯部分：紧致化空间度规涨落
    \item 汤川耦合：几何重叠积分
