...
本节介绍从10维几何严格推导完整标准模型（加引力）拉格朗日量。从克利福德代数 $Cl(1,9)$ 出发，我们通过谱维流动框架推导拉格朗日量中的每一项。
...
\subsection{双向谱维流：10D内空间与4D互反空间的耦合}

\subsubsection{扭转场耦合机制}

在CTUFT中，10维内空间和4维互反空间是平行存在的，通过**扭转场**耦合：
\begin{itemize}
    \item $x^\mu$：4维互反空间（$\mu = 0,1,2,3$）——我们的物理时空
    \item 10维内空间：与互反空间对偶，高能时耦合开放
\end{itemize}

**扭转场本身就是耦合界面**，不是独立的"膜"。它将10维谱维流动转化为在4维互反空间内流动的能量-信息。从4维视角，我们无法直接感知10维内空间的拓扑结构；只能通过谱维流动 $d_s(E)$ 间接观测。

4维中的有效度规接收来自扭转场耦合的贡献：
\begin{equation}
    ds_{4}^2 = g_{\mu\nu}dx^\mu dx^\nu + \text{（扭转修正）}
\end{equation}
...
\begin{itemize}
    \item $g_{\mu\nu}$ 是4维度规
    \item 扭转场介导能量-信息在两个空间之间传输
    \item 谱维流动 $d_s(E)$ 将10维信息以能量形式带入4维
\end{itemize}

\subsubsection{双向谱维流公式}
...
\begin{enumerate}
    ...
    \item 通过双向谱维流框架的有效4维理论
    \item 引力部分：爱因斯坦-希尔伯特 + 挠率修正
    \item 规范部分：来自内空间对称性的 $U(1) \times SU(2) \times SU(3)$
