...
\begin{insight}[$Cl(1,9)$ 与 $\Cl(3,1)$ 的关系]
第\ref{sec:math}节引入的 $\Cl(3,1)$ 描述我们观测到的4维互反空间的局部结构，而 $Cl(1,9)$ 描述10维内空间的全局代数结构。两者的关系是：当谱维从 $d_s = 10$ 流动到 $d_s = 4$ 时，$Cl(1,9)$ 的某些子结构通过层投影约化为 $\Cl(3,1)$ 的扩展形式。具体而言：
\begin{itemize}
    \item $Cl(1,9)$ 包含一个自然的 $\Cl(3,1)$ 子代数，对应4维时空的旋量结构
    \item 剩余6个生成元对应内空间的额外自由度，产生规范对称性
    \item 扭转场 $\torsion^\alpha_{\ \mu\nu}$ 提供了从内空间到互反空间的几何连接
\end{itemize}
因此，$Cl(1,9)$ 是统一所有力的"母代数"，而 $\Cl(3,1)$ 是我们观测到低能世界的有效代数。
\end{insight}
