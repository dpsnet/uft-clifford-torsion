...
\begin{itemize}
    ...
    \item \textbf{蒸发的终点}：黑洞蒸发的最终状态是什么？
\end{itemize}

CTUFT通过分形-扭转几何和10维内空间解决所有三个问题。

\subsection{经典广义相对论中的奇点问题}
...
\subsubsection{奇点消除通过内空间展开}

\begin{insight}[奇点作为视角效应]
$r = 0$ 处的"奇点"不是无限物理密度的点。它是4D观察者对内空间展开以容纳能量的区域的视角。
\end{insight}
...
当 $M \to M_{\text{crit}} \sim M_P/\tau_0$ 时，蒸发行为改变：
\begin{itemize}
    \item 如果 $\tau_0 \ll 1$：黑洞在最终爆发（伽马射线）中完全蒸发
...
对于旋转、带电黑洞，CTUFT度规推广为Kerr-Newman扭转修正：
\begin{equation}
...
其中 $\mathcal{N}_{\text{int}}$ 是内空间微观态数目。
...
\begin{enumerate}
    ...
    \item \textbf{防火墙悖论}：视界处无高能辐射，等效原理被保留
\end{enumerate}
