...
In the Standard Model, fermion masses arise through Yukawa couplings to the Higgs field, requiring about 19 arbitrary parameters. CTUFT derives all fermion masses from geometric principles, depending only on the single parameter $\tau_0$.
...
\begin{tabular}{lccc}
\hline
\textbf{Quark} & \textbf{CTUFT Prediction} & \textbf{Experimental} & \textbf{Error} \\
\hline
$u$ (up) & 2.2 & 2.2 & 0\% \\
$d$ (down) & 4.7 & 4.7 & 0\% \\
...
\begin{tabular}{lccc}
\hline
\textbf{Lepton} & \textbf{CTUFT Prediction} & \textbf{Experimental} & \textbf{Error} \\
\hline
...
CTUFT prediction: $J = \tau_0^3/6 \approx 2.1 \times 10^{-5}$
...
\begin{itemize}
    ...
    \item \textbf{Axion-like particles}: Goldstone bosons from internal space
...
CTUFT predicts baryon number violation at very high energies:
\begin{equation}
...
CTUFT predicts all fermion masses from geometric principles:
\begin{enumerate}
