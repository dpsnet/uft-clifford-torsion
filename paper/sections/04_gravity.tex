...
\caption{Six gravitational wave polarization modes predicted by the unified field theory. The vector modes ($h_x$, $h_y$) have amplitude $\sim \tau_0/2$ and the scalar modes ($h_b$, $h_l$) have amplitude $\sim \tau_0^2/3$ relative to the standard tensor modes.}
...
\begin{enumerate}
    ...
    \item \textbf{$h_x$ (Vector-x)}: $e_{xz} = e_{zx} = 1$ — CTUFT prediction
    \item \textbf{$h_y$ (Vector-y)}: $e_{yz} = e_{zy} = 1$ — CTUFT prediction
    \item \textbf{$h_b$ (Breathing)}: $e_{xx} = e_{yy} = 1$ — CTUFT prediction
    \item \textbf{$h_l$ (Longitudinal)}: $e_{zz} = 1$ — CTUFT prediction
\end{enumerate}
