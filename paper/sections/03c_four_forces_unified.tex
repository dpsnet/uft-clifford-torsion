...
\begin{insight}[Duality between $Cl(1,9)$ and $\Cl(3,1)$]
The $\Cl(3,1)$ introduced in Section~\ref{sec:math} describes the algebraic structure of 4D reciprocal space, while $Cl(1,9)$ describes the algebraic structure of 10D internal space. These two algebras form a \textbf{duality mirror} relationship through the \textbf{interface} between 4D spacetime and 10D internal space:
\begin{itemize}
    \item The generators of $\Cl(3,1)$ for 4D reciprocal space are dual to those of $Cl(1,9)$ for 10D internal space
    \item Energy flowing inward in reciprocal space expands outward in internal space (duality principle)
    \item The torsion field $\torsion^\alpha_{\ \mu\nu}$ serves as the interface field connecting the two spaces, implementing the dual mapping of generators
\end{itemize}
This duality is not a subalgebra inclusion but an algebraic manifestation of the reciprocal space-internal space duality: the 4 generators of $\Cl(3,1)$ correspond to a specific 4-dimensional submanifold in $Cl(1,9)$, while the remaining generators of $Cl(1,9)$ describe the additional degrees of freedom of internal space (producing gauge symmetries).
\end{insight}
...
\end{tikzpicture}
\caption{Geometric hierarchy of force unification in CTUFT. All four forces originate from the $Cl(1,9)$ algebra and emerge through sheaf projection with characteristic twisting angles.}
...
The Einstein-Hilbert action is the low-energy effective action of the full CTUFT:
...
\end{semilogyaxis}
\end{tikzpicture}
\caption{Renormalization group running of gauge couplings in CTUFT. All three forces unify at $E_c \sim 10^{21}$ GeV with corrections from torsion-dependent terms.}
