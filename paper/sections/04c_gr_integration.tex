% Section: CTUFT与广义相对论的融合
\section{Integration with General Relativity}
\label{sec:gr_integration}

\subsection{The Hierarchy of Theories}

Our unified field theory (CTUFT) stands in a hierarchical relationship with Einstein's General Relativity (GR) and the Einstein-Cartan theory:

\begin{equation}
    \text{CTUFT} \supset \text{Einstein-Cartan} \supset \text{General Relativity}
    \label{eq:theory_hierarchy}
\end{equation}

This hierarchy is realized through the torsion field limit:

\begin{theorem}[GR Limit of CTUFT]
In the limit $\tau_0 \to 0$, the CTUFT action reduces to the Einstein-Hilbert action:
\begin{equation}
    S_{CTUFT} \xrightarrow{\tau_0 \to 0} S_{GR} = \int d^4x \sqrt{-g} \left[\frac{R}{2\kappa} + \mathcal{L}_{matter}\right]
\end{equation}
\end{theorem}
...
The CTUFT action is:
\begin{equation}
    S_{CTUFT} = \int d^4x \sqrt{-g} \left[\frac{R}{2\kappa} + \frac{\mathcal{T}_{\mu\nu\alpha}\mathcal{T}^{\mu\nu\alpha}}{4\tau_0} + \mathcal{L}_{matter}\right]
\end{equation}
...
\begin{itemize}
    ...
    \item \textbf{Small $\tau_0$} (equivalently, large torsion): Torsion effects become significant. CTUFT deviations from GR become observable.
\end{itemize}
...
In all conventional astrophysical scenarios, the correction is negligible ($\delta\tau \ll 1$), making CTUFT indistinguishable from GR.
...
General Relativity concerns itself with the \textbf{topological dimension} (fixed at 4). The CTUFT adds dynamics to the \textbf{spectral dimension} without affecting the topological structure.
\end{proposition}
...
While CTUFT reproduces GR in the classical limit, it makes distinct predictions for gravitational wave physics:

\begin{theorem}[Gravitational Wave Polarizations]
General Relativity predicts 2 polarization modes ($h_+, h_\times$). CTUFT predicts 6 polarization modes:
\begin{itemize}
...
The Friedmann equations receive CTUFT corrections:
...
The relationship between CTUFT and GR mirrors that between GR and Newtonian mechanics:
...
CTUFT & Quantum gravity, high energy & $\tau_0 \neq 0$ \\
\hline
\end{tabular}
\end{center}

\begin{remark}[Completing Einstein's Dream]
Einstein sought to geometrize all of physics. General Relativity geometrized gravity. CTUFT extends this geometrization to all fundamental forces. This is not a replacement of Einstein's work but its completion.
\end{remark}

\subsection{Summary}

CTUFT integrates perfectly with General Relativity:
\begin{enumerate}
    \item CTUFT $\supset$ Einstein-Cartan $\supset$ GR (hierarchical relationship)
    \item In the limit $\tau_0 \to 0$, CTUFT reduces exactly to GR
...
Einstein's General Relativity is the low-energy, weak-field limit of CTUFT, just as Newtonian mechanics is the low-speed limit of GR. This is the perfect integration of the two frameworks.
