...
\subsection{Dimensional Reduction: 10D → 4D}

\subsubsection{The Kaluza-Klein Ansatz}

We decompose the 10D coordinates as $x^M = (x^\mu, y^a)$ where:
\begin{itemize}
    \item $x^\mu$: 4D reciprocal space ($\mu = 0,1,2,3$)
    \item $y^a$: 6D internal directions ($a = 4,\ldots,9$)
\end{itemize}
...
\begin{itemize}
    \item $g_{\mu\nu}$ is the 4D metric
    \item $g_{ab}$ is the internal metric
    \item $A^a_\mu$ are the gauge fields (Kaluza-Klein vectors)
    \item $\Phi$ is the dilaton
\end{itemize}

\subsubsection{Gauge Symmetry Emergence}

The isometries of the internal space become gauge symmetries in 4D:
\begin{equation}
...
\subsection{The Complete UFT Lagrangian}

Including gravity and torsion:

\begin{equation}
    \mathcal{L}_{\text{UFT}} = \mathcal{L}_{\text{SM}} + \mathcal{L}_{\text{grav}} + \mathcal{L}_{\tau}
\end{equation}
...
\begin{table}[htbp]
\centering
\caption{SM Parameters Derived from UFT Geometry}
\begin{tabular}{@{}lcc@{}}
\toprule
...
The Standard Model emerges as the low-energy ($E \ll E_c$) effective theory of the 10D UFT geometry, with all parameters calculable from the single constant $\tau_0 = 0.005$.
