...
\hline
Quick estimates & $\checkmark$ Ideal ($\sim$1 min) & Warning: Overkill ($\sim$1 hour) \\
Order-of-magnitude & $\checkmark$ Accurate to $\sim$30\% & $\checkmark$ Accurate to $\sim$10\% \\
Precision predictions & $\times$ Inadequate & $\checkmark$ Required \\
CP violation studies & $\times$ No prediction & $\checkmark$ $\delta_{CP}$ predicted \\
Neutrino physics & Warning: Needs extension & $\checkmark$ Unified framework \\
Rare decay rates & $\times$ No prediction & $\checkmark$ Calculable \\
\hline
\end{tabular}
